/**
 * @license lodash <https://lodash.com/> Copyright JS Foundation and other
...
;(function() {
  ...
  /**
	 ...
	 * true // Create a suped-up `defer` in Node.js. var defer =
  ...
  var runInContext = (function runInContext(context) {
	 ...
	 * and functions can be chained together. Methods that retrieve a single
	 * value or may return a primitive value will automatically end the chain
	 * sequence and return the unwrapped value. Otherwise, the value must be
	 ...
	 * `assignInWith`, `assignWith`, `at`, `before`, `bind`, `bindAll`,
	 * `bindKey`, `castArray`, `chain`, `chunk`, `commit`, `compact`, `concat`,
	 * `conforms`, `constant`, `countBy`, `create`, `curry`, `debounce`,
	 * `defaults`, `defaultsDeep`, `defer`, `delay`, `difference`,
	 * `differenceBy`, `differenceWith`, `drop`, `dropRight`, `dropRightWhile`,
	 * `dropWhile`, `extend`, `extendWith`, `fill`, `filter`, `flatMap`,
	 * `flatMapDeep`, `flatMapDepth`, `flatten`, `flattenDeep`, `flattenDepth`,
	 * `flip`, `flow`, `flowRight`, `fromPairs`, `functions`, `functionsIn`,
	 * `groupBy`, `initial`, `intersection`, `intersectionBy`,
	 * `intersectionWith`, `invert`, `invertBy`, `invokeMap`, `iteratee`,
	 * `keyBy`, `keys`, `keysIn`, `map`, `mapKeys`, `mapValues`, `matches`,
	 ...
	 * `takeWhile`, `tap`, `throttle`, `thru`, `toArray`, `toPairs`,
	 * `toPairsIn`, `toPath`, `toPlainObject`, `transform`, `unary`, `union`,
	 * `unionBy`, `unionWith`, `uniq`, `uniqBy`, `uniqWith`, `unset`, `unshift`,
	 * `unzip`, `unzipWith`, `update`, `updateWith`, `values`, `valuesIn`,
	 ...
	 * `lt`, `lte`, `max`, `maxBy`, `mean`, `meanBy`, `min`, `minBy`,
	 * `multiply`, `noConflict`, `noop`, `now`, `nth`, `pad`, `padEnd`,
	 * `padStart`, `parseInt`, `pop`, `random`, `reduce`, `reduceRight`,
      ...
      for (var key in value) {
        if ((inherited || hasOwnProperty.call(value, key)) &&
            !(skipIndexes && (
               ...
               // PhantomJS 2 has enumerable non-index properties on typed
				// arrays.
	 ...
	 * @param {Object}
	 *            [stack] Tracks traversed objects and their clone counterparts.
	 ...
	 * a `comparator` to determine the extremum value.
	 * 
	 * @private
	 * @param {Array}
	 *            array The array to iterate over.
	 ...
	 * @param {boolean}
	 *            [bitmask] The bitmask of comparison flags. The bitmask may be
	 *            composed of the following flags: 1 - Unordered comparison 2 -
	 *            Partial comparison
	 * @param {Object}
    ...
    /**
	 * The base implementation of `_.keysIn` which doesn't treat sparse arrays
	 * as dense.
	 * 
	 * @private
	 * @param {Object}
	 ...
	 * which invokes `iteratee` for `value` and each element of `array` to
	 ...
	 * @private
	 * @param {number|Object}
	 *            id The timer id or timeout object of the timer to clear.
	 */
	 ...
	 *            func The function to wrap.
	 * @param {number}
	 *            bitmask The bitmask flags. See `createWrap` for more details.
	 * @param {*}
	 *            [thisArg] The `this` binding of `func`.
	 ...
	 *            func The function or method name to wrap.
	 * @param {number}
	 *            bitmask The bitmask flags. The bitmask may be composed of the
	 ...
	 *            customizer The function to customize comparisons.
	 * @param {number}
	 *            bitmask The bitmask of comparison flags. See `baseIsEqual` for
	 *            more details.
	 * @param {Object}
    ...
    /**
	 * Creates an array of the own and inherited enumerable symbol properties of
	 * `object`.
	 * 
	 * @private
	 * @param {Object}
    ...
    // Fallback for data views, maps, sets, and weak maps in IE 11 and promises
	// in Node.js < 6.
	 ...
	 * `_.ary` and `_.rearg` modify function arguments, making the order in
	 * which they are executed important, preventing the merging of metadata.
	 * However, we make an exception for a safe combined case where curried
	 ...
	 * 2070](https://bugs.chromium.org/p/v8/issues/detail?id=2070) for more
	 * details.
	 * 
	 * @private
	 * @param {Function}
	 ...
	 * `reference` with wrapper details in a comment at the top of the source
	 * body.
	 * 
	 * @private
	 * @param {Function}
    ...
    /**
	 * Creates a function that'll short out and invoke `identity` instead of
	 * `func` when it's called `HOT_COUNT` or more times in `HOT_SPAN`
	 * milliseconds.
	 * 
	 * @private
	 * @param {Function}
	 ...
	 *            array The array to shuffle.
	 * @param {number}
	 *            [size=array.length] The size of `array`.
	 * @returns {Array} Returns `array`.
	 */
	 ...
	 * @example
	 * 
	 * _.differenceBy([2.1, 1.2], [2.3, 3.4], Math.floor); // => [1.2] // The
	 * `_.property` iteratee shorthand. _.differenceBy([{ 'x': 2 }, { 'x': 1 }], [{
	 * 'x': 1 }], 'x'); // => [{ 'x': 2 }]
	 */
	 ...
	 * and references of result values are determined by the first array. The
	 ...
	 * 
	 * _.differenceWith(objects, [{ 'x': 1, 'y': 2 }], _.isEqual); // => [{ 'x':
	 * 2, 'y': 1 }]
	 */
	 ...
	 * 
	 * _.dropRightWhile(users, function(o) { return !o.active; }); // => objects
	 * for ['barney'] // The `_.matches` iteratee shorthand.
	 * _.dropRightWhile(users, { 'user': 'pebbles', 'active': false }); // =>
	 * objects for ['barney', 'fred'] // The `_.matchesProperty` iteratee
	 * shorthand. _.dropRightWhile(users, ['active', false]); // => objects for
	 * ['barney'] // The `_.property` iteratee shorthand.
	 * _.dropRightWhile(users, 'active'); // => objects for ['barney', 'fred',
	 * 'pebbles']
	 */
	 ...
	 * 
	 * _.dropWhile(users, function(o) { return !o.active; }); // => objects for
	 * ['pebbles'] // The `_.matches` iteratee shorthand. _.dropWhile(users, {
	 * 'user': 'barney', 'active': false }); // => objects for ['fred',
	 ...
	 * 'active': false }); // => 1 // The `_.matchesProperty` iteratee
	 * shorthand. _.findIndex(users, ['active', false]); // => 0 // The
	 * `_.property` iteratee shorthand. _.findIndex(users, 'active'); // => 2
	 */
	 ...
	 * 2 // The `_.matches` iteratee shorthand. _.findLastIndex(users, { 'user':
	 * 'barney', 'active': true }); // => 0 // The `_.matchesProperty` iteratee
	 * shorthand. _.findLastIndex(users, ['active', false]); // => 2 // The
	 * `_.property` iteratee shorthand. _.findLastIndex(users, 'active'); // =>
	 * 0
	 */
	 ...
	 * @example
	 * 
	 * _.indexOf([1, 2, 1, 2], 2); // => 1 // Search from the `fromIndex`.
	 ...
	 * @example
	 * 
	 * _.intersectionBy([2.1, 1.2], [2.3, 3.4], Math.floor); // => [2.1] // The
	 * `_.property` iteratee shorthand. _.intersectionBy([{ 'x': 1 }], [{ 'x': 2 }, {
	 * 'x': 1 }], 'x'); // => [{ 'x': 1 }]
	 */
	 ...
	 * @example
	 * 
	 * var objects = [{ 'x': 1, 'y': 2 }, { 'x': 2, 'y': 1 }]; var others = [{
	 * 'x': 1, 'y': 1 }, { 'x': 1, 'y': 2 }];
	 * 
	 ...
	 * @example
	 * 
	 * _.lastIndexOf([1, 2, 1, 2], 2); // => 3 // Search from the `fromIndex`.
    ...
    /**
	 * This method is like `_.pullAll` except that it accepts `iteratee` which
	 ...
	 * @example
	 * 
	 * var array = ['a', 'b', 'c', 'd']; var pulled = _.pullAt(array, [1, 3]);
	 * 
	 ...
	 * 
	 * **Note:** Unlike `_.filter`, this method mutates `array`. Use `_.pull` to
	 * pull elements from an array by value.
	 * 
	 * @static
	 * @memberOf _
	 * @since 2.0.0
	 ...
	 * 0 // The `_.property` iteratee shorthand. _.sortedIndexBy(objects, { 'x':
	 * 4 }, 'x'); // => 0
	 */
    ...
    /**
	 * This method is like `_.indexOf` except that it performs a binary search
	 * on a sorted `array`.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * 1 // The `_.property` iteratee shorthand. _.sortedLastIndexBy(objects, {
	 * 'x': 4 }, 'x'); // => 1
	 */
	 ...
	 * 'active': false }, { 'user': 'pebbles', 'active': false } ];
	 * 
	 * _.takeRightWhile(users, function(o) { return !o.active; }); // => objects
	 * for ['fred', 'pebbles'] // The `_.matches` iteratee shorthand.
	 * _.takeRightWhile(users, { 'user': 'pebbles', 'active': false }); // =>
	 * objects for ['pebbles'] // The `_.matchesProperty` iteratee shorthand.
	 * _.takeRightWhile(users, ['active', false]); // => objects for ['fred',
	 * 'pebbles'] // The `_.property` iteratee shorthand.
	 ...
	 * 'active': false}, { 'user': 'pebbles', 'active': true } ];
	 * 
	 * _.takeWhile(users, function(o) { return !o.active; }); // => objects for
	 * ['barney', 'fred'] // The `_.matches` iteratee shorthand.
	 * _.takeWhile(users, { 'user': 'barney', 'active': false }); // => objects
	 * for ['barney'] // The `_.matchesProperty` iteratee shorthand.
	 * _.takeWhile(users, ['active', false]); // => objects for ['barney',
	 * 'fred'] // The `_.property` iteratee shorthand. _.takeWhile(users,
	 * 'active'); // => []
	 */
	 ...
	 * array in which the value occurs. The iteratee is invoked with one
	 * argument: (value).
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * @example
	 * 
	 * _.unionBy([2.1], [1.2, 2.3], Math.floor); // => [2.1, 1.2] // The
	 * `_.property` iteratee shorthand. _.unionBy([{ 'x': 1 }], [{ 'x': 2 }, {
	 * 'x': 1 }], 'x'); // => [{ 'x': 1 }, { 'x': 2 }]
	 */
	 ...
	 * the first array in which the value occurs. The comparator is invoked with
	 ...
	 * 'x': 1, 'y': 1 }, { 'x': 1, 'y': 2 }];
	 * 
	 * _.unionWith(objects, others, _.isEqual); // => [{ 'x': 1, 'y': 2 }, {
	 * 'x': 2, 'y': 1 }, { 'x': 1, 'y': 1 }]
	 */
	 ...
	 * @example
	 * 
	 * _.uniqBy([2.1, 1.2, 2.3], Math.floor); // => [2.1, 1.2] // The
	 * `_.property` iteratee shorthand. _.uniqBy([{ 'x': 1 }, { 'x': 2 }, { 'x':
	 * 1 }], 'x'); // => [{ 'x': 1 }, { 'x': 2 }]
	 */
	 ...
	 * }];
	 * 
	 * _.uniqWith(objects, _.isEqual); // => [{ 'x': 1, 'y': 2 }, { 'x': 2, 'y':
	 * 1 }]
	 */
	 ...
	 * @example
	 * 
	 * var zipped = _.zip(['a', 'b'], [1, 2], [true, false]); // => [['a', 1,
	 * true], ['b', 2, false]]
	 * 
	 ...
	 * with the elements of each group: (...group).
	 * 
	 * @static
	 * @memberOf _
	 * @since 3.8.0
	 ...
	 * @example
	 * 
	 * _.xorBy([2.1, 1.2], [2.3, 3.4], Math.floor); // => [1.2, 3.4] // The
	 * `_.property` iteratee shorthand. _.xorBy([{ 'x': 1 }], [{ 'x': 2 }, {
	 * 'x': 1 }], 'x'); // => [{ 'x': 2 }]
	 */
	 ...
	 * 'x': 1, 'y': 1 }, { 'x': 1, 'y': 2 }];
	 * 
	 * _.xorWith(objects, others, _.isEqual); // => [{ 'x': 2, 'y': 1 }, { 'x':
	 * 1, 'y': 1 }]
	 */
	 ...
	 * @example
	 * 
	 * _.zip(['a', 'b'], [1, 2], [true, false]); // => [['a', 1, true], ['b', 2,
	 * false]]
	 */
    var zip = baseRest(unzip);

    /**
	 * This method is like `_.fromPairs` except that it accepts two arrays, one
	 * of property identifiers and one of corresponding values.
	 * 
	 * @static
	 * @memberOf _
	 * @since 0.4.0
	 ...
	 * @example
	 * 
	 * _.zipObjectDeep(['a.b[0].c', 'a.b[1].d'], [1, 2]); // => { 'a': { 'b': [{
	 * 'c': 1 }, { 'd': 2 }] } }
	 */
	 ...
	 *            [arrays] The arrays to process.
	 * @param {Function}
	 *            [iteratee=_.identity] The function to combine grouped values.
	 * @returns {Array} Returns the new array of grouped elements.
	 * @example
	 * 
	 * _.zipWith([1, 2], [10, 20], [100, 200], function(a, b, c) { return a + b +
	 * c; }); // => [111, 222]
	 */
	 ...
	 * 
	 * var youngest = _ .chain(users) .sortBy('age') .map(function(o) { return
	 * o.user + ' is ' + o.age; }) .head() .value(); // => 'pebbles is 1'
	 */
	 ...
	 * invoked with one argument; (value). The purpose of this method is to "tap
	 * into" a method chain sequence in order to modify intermediate results.
	 * 
	 * @static
	 * @memberOf _
	 * @since 0.1.0
	 ...
	 * @example
	 * 
	 * _([1, 2, 3]) .tap(function(array) { // Mutate input array. array.pop(); })
	 * .reverse() .value(); // => [2, 1]
	 */
	 ...
	 * 
	 * var users = [ { 'user': 'barney', 'age': 36 }, { 'user': 'fred', 'age':
	 * 40 } ]; // A sequence without explicit chaining. _(users).head(); // => {
	 * 'user': 'barney', 'age': 36 } // A sequence with explicit chaining.
	 * _(users) .chain() .head() .pick('user') .value(); // => { 'user':
	 * 'barney' }
	 */
	 ...
	 * 
	 * wrapped.next(); // => { 'done': true, 'value': undefined }
	 */
	 ...
	 * @example
	 * 
	 * _.countBy([6.1, 4.2, 6.3], Math.floor); // => { '4': 1, '6': 2 } // The
	 * `_.property` iteratee shorthand. _.countBy(['one', 'two', 'three'],
	 * 'length'); // => { '3': 2, '5': 1 }
	 */
	 ...
	 * `collection`. Iteration is stopped once `predicate` returns falsey. The
	 * predicate is invoked with three arguments: (value, index|key,
	 * collection).
	 * 
	 * **Note:** This method returns `true` for [empty
	 * collections](https://en.wikipedia.org/wiki/Empty_set) because [everything
	 * is true](https://en.wikipedia.org/wiki/Vacuous_truth) of elements of
	 * empty collections.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * 'fred', 'age': 40, 'active': false } ]; // The `_.matches` iteratee
	 * shorthand. _.every(users, { 'user': 'barney', 'active': false }); // =>
	 * false // The `_.matchesProperty` iteratee shorthand. _.every(users,
	 * ['active', false]); // => true // The `_.property` iteratee shorthand.
	 ...
	 * elements `predicate` returns truthy for. The predicate is invoked with
	 ...
	 * 
	 * _.filter(users, function(o) { return !o.active; }); // => objects for
	 * ['fred'] // The `_.matches` iteratee shorthand. _.filter(users, { 'age':
	 * 36, 'active': true }); // => objects for ['barney'] // The
	 * `_.matchesProperty` iteratee shorthand. _.filter(users, ['active',
	 * false]); // => objects for ['fred'] // The `_.property` iteratee
	 * shorthand. _.filter(users, 'active'); // => objects for ['barney']
	 */
	 ...
	 * 'active': true } ];
	 * 
	 * _.find(users, function(o) { return o.age < 40; }); // => object for
	 * 'barney' // The `_.matches` iteratee shorthand. _.find(users, { 'age': 1,
	 * 'active': true }); // => object for 'pebbles' // The `_.matchesProperty`
	 * iteratee shorthand. _.find(users, ['active', false]); // => object for
	 * 'fred' // The `_.property` iteratee shorthand. _.find(users, 'active'); // =>
	 * object for 'barney'
	 */
	 ...
	 * @example
	 * 
	 * _.forEach([1, 2], function(value) { console.log(value); }); // => Logs
	 * `1` then `2`.
	 * 
	 ...
	 * @example
	 * 
	 * _.groupBy([6.1, 4.2, 6.3], Math.floor); // => { '4': [4.2], '6': [6.1,
	 * 6.3] } // The `_.property` iteratee shorthand. _.groupBy(['one', 'two',
	 ...
	 * [`SameValueZero`](http://ecma-international.org/ecma-262/7.0/#sec-samevaluezero)
	 * is used for equality comparisons. If `fromIndex` is negative, it's used
	 ...
	 * 
	 * _.invokeMap([123, 456], String.prototype.split, ''); // => [['1', '2',
	 * '3'], ['4', '5', '6']]
	 */
	 ...
	 * @example
	 * 
	 * var array = [ { 'dir': 'left', 'code': 97 }, { 'dir': 'right', 'code':
	 * 100 } ];
	 * 
	 * _.keyBy(array, function(o) { return String.fromCharCode(o.code); }); // => {
	 * 'a': { 'dir': 'left', 'code': 97 }, 'd': { 'dir': 'right', 'code': 100 } }
	 * 
	 * _.keyBy(array, 'dir'); // => { 'left': { 'dir': 'left', 'code': 97 },
	 ...
	 * `rangeRight`, `repeat`, `sampleSize`, `slice`, `some`, `sortBy`, `split`,
	 * `take`, `takeRight`, `template`, `trim`, `trimEnd`, `trimStart`, and
	 * `words`
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * 
	 * _.map({ 'a': 4, 'b': 8 }, square); // => [16, 64] (iteration order is not
	 * guaranteed)
	 * 
	 * var users = [ { 'user': 'barney' }, { 'user': 'fred' } ]; // The
	 * `_.property` iteratee shorthand. _.map(users, 'user'); // => ['barney',
	 * 'fred']
	 */
	 ...
	 * orders of the iteratees to sort by. If `orders` is unspecified, all
	 * values are sorted in ascending order. Otherwise, specify an order of
	 * "desc" for descending or "asc" for ascending sort order of corresponding
	 * values.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * 34 }, { 'user': 'fred', 'age': 40 }, { 'user': 'barney', 'age': 36 } ]; //
	 * Sort by `user` in ascending order and by `age` in descending order.
	 * _.orderBy(users, ['user', 'age'], ['asc', 'desc']); // => objects for
	 ...
	 * 'active': false } ];
	 * 
	 * _.partition(users, function(o) { return o.active; }); // => objects for
	 * [['fred'], ['barney', 'pebbles']] // The `_.matches` iteratee shorthand.
	 * _.partition(users, { 'age': 1, 'active': false }); // => objects for
	 * [['pebbles'], ['barney', 'fred']] // The `_.matchesProperty` iteratee
	 * shorthand. _.partition(users, ['active', false]); // => objects for
	 * [['barney', 'pebbles'], ['fred']] // The `_.property` iteratee shorthand.
	 * _.partition(users, 'active'); // => objects for [['fred'], ['barney',
	 * 'pebbles']]
	 */
	 ...
	 * `accumulator` is not given, the first element of `collection` is used as
	 * the initial value. The iteratee is invoked with four arguments:
	 ...
	 * 
	 * _.reject(users, function(o) { return !o.active; }); // => objects for
	 * ['fred'] // The `_.matches` iteratee shorthand. _.reject(users, { 'age':
	 * 40, 'active': true }); // => objects for ['barney'] // The
	 * `_.matchesProperty` iteratee shorthand. _.reject(users, ['active',
	 * false]); // => objects for ['fred'] // The `_.property` iteratee
	 * shorthand. _.reject(users, 'active'); // => objects for ['barney']
	 */
    ...
    /**
	 * Gets `n` random elements at unique keys from `collection` up to the size
	 * of `collection`.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * 
	 * var users = [ { 'user': 'barney', 'active': true }, { 'user': 'fred',
	 * 'active': false } ]; // The `_.matches` iteratee shorthand. _.some(users, {
	 * 'user': 'barney', 'active': false }); // => false // The
	 * `_.matchesProperty` iteratee shorthand. _.some(users, ['active', false]); // =>
	 * true // The `_.property` iteratee shorthand. _.some(users, 'active'); // =>
	 * true
	 */
	 ...
	 * 
	 * var users = [ { 'user': 'fred', 'age': 48 }, { 'user': 'barney', 'age':
	 * 36 }, { 'user': 'fred', 'age': 40 }, { 'user': 'barney', 'age': 34 } ];
	 * 
	 * _.sortBy(users, [function(o) { return o.user; }]); // => objects for
	 * [['barney', 36], ['barney', 34], ['fred', 48], ['fred', 40]]
	 * 
	 * _.sortBy(users, ['user', 'age']); // => objects for [['barney', 34],
	 ...
	 * @example
	 * 
	 * jQuery(element).on('click', _.before(5, addContactToList)); // => Allows
	 ...
	 * @example
	 * 
	 * function greet(greeting, punctuation) { return greeting + ' ' + this.user +
	 * punctuation; }
	 * 
	 * var object = { 'user': 'fred' };
	 * 
	 * var bound = _.bind(greet, object, 'hi'); bound('!'); // => 'hi fred!' //
	 * Bound with placeholders. var bound = _.bind(greet, object, _, '!');
	 * bound('hi'); // => 'hi fred!'
	 */
	 ...
	 * 
	 * bound('!'); // => 'hiya fred!' // Bound with placeholders. var bound =
	 * _.bindKey(object, 'greet', _, '!'); bound('hi'); // => 'hiya fred!'
	 */
	 ...
	 * curried(1, 2)(3); // => [1, 2, 3]
	 * 
	 * curried(1, 2, 3); // => [1, 2, 3] // Curried with placeholders.
	 ...
	 * to cancel delayed `func` invocations and a `flush` method to immediately
	 * invoke them. Provide `options` to indicate whether `func` should be
	 * invoked on the leading and/or trailing edge of the `wait` timeout. The
	 * `func` is invoked with the last arguments provided to the debounced
	 ...
	 *          invoked once after 1 second of debounced calls. var debounced =
	 ...
	 * 'deferred' after one millisecond.
	 */
	 ...
	 * argument provided to the memoized function is used as the map cache key.
	 ...
	 * `func` is invoked with the `this` binding and arguments of the created
	 * function.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * function square(n) { return n * n; }
	 * 
	 * var func = _.overArgs(function(x, y) { return [x, y]; }, [square,
	 * doubled]);
	 * 
	 * func(9, 3); // => [81, 6]
	 * 
	 ...
	 * arguments it receives. This method is like `_.bind` except it does
	 ...
	 * 'hello fred' // Partially applied with placeholders. var greetFred =
	 * _.partial(greet, _, 'fred'); greetFred('hi'); // => 'hi fred'
	 */
	 ...
	 * fred' // Partially applied with placeholders. var sayHelloTo =
	 * _.partialRight(greet, 'hello', _); sayHelloTo('fred'); // => 'hello fred'
	 */
	 ...
	 * @example
	 * 
	 * var rearged = _.rearg(function(a, b, c) { return [a, b, c]; }, [2, 0,
	 * 1]);
	 * 
	 * rearged('b', 'c', 'a') // => ['a', 'b', 'c']
	 */
	 ...
	 * @example
	 * 
	 * var say = _.rest(function(what, names) { return what + ' ' +
	 ...
	 *          Invoke `renewToken` when the click event is fired, but not more
	 *          than once every 5 minutes. var throttled =
	 *          _.throttle(renewToken, 300000, { 'trailing': false });
	 *          jQuery(element).on('click', throttled); // Cancel the trailing
	 *          throttled invocation. jQuery(window).on('popstate',
	 *          throttled.cancel);
	 */
	 ...
	 * to those provided to the `wrapper`. The wrapper is invoked with the
	 ...
	 * _.castArray(); // => []
	 * 
	 * var array = [1, 2, 3]; console.log(_.castArray(array) === array); // =>
	 * true
	 */
    function castArray() {
      if (!arguments.length) {
	 ...
	 * `Object` objects, regexes, sets, strings, symbols, and typed arrays. The
	 * own enumerable properties of `arguments` objects are cloned as plain
	 ...
	 * var objects = [{ 'a': 1 }, { 'b': 2 }];
	 * 
	 * var shallow = _.clone(objects); console.log(shallow[0] === objects[0]); // =>
	 * true
	 */
	 ...
	 * is invoked with up to four arguments; (value [, index|key, object,
	 * stack]).
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * @example
	 * 
	 * function customizer(value) { if (_.isElement(value)) { return
	 * value.cloneNode(false); } }
	 * 
	 * var el = _.cloneWith(document.body, customizer);
	 * 
	 * console.log(el === document.body); // => false console.log(el.nodeName); // =>
	 * 'BODY' console.log(el.childNodes.length); // => 0
	 */
	 ...
	 * var objects = [{ 'a': 1 }, { 'b': 2 }];
	 * 
	 * var deep = _.cloneDeep(objects); console.log(deep[0] === objects[0]); // =>
	 * false
	 */
	 ...
	 * 
	 * console.log(el === document.body); // => false console.log(el.nodeName); // =>
	 * 'BODY' console.log(el.childNodes.length); // => 20
	 */
	 ...
	 * comparisons are handled by the method instead. The `customizer` is
	 * invoked with up to six arguments: (objValue, othValue [, index|key,
	 * object, other, stack]).
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * 
	 * function customizer(objValue, othValue) { if (isGreeting(objValue) &&
	 * isGreeting(othValue)) { return true; } }
	 * 
	 ...
	 * is invoked to compare values. If `customizer` returns `undefined`,
	 * comparisons are handled by the method instead. The `customizer` is
	 * invoked with five arguments: (objValue, srcValue, index|key, object,
	 * source).
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * function isGreeting(value) { return /^h(?:i|ello)$/.test(value); }
	 * 
	 * function customizer(objValue, srcValue) { if (isGreeting(objValue) &&
	 * isGreeting(srcValue)) { return true; } }
	 * 
	 ...
	 * Foo.prototype.b = 2; Bar.prototype.d = 4;
	 * 
	 * _.assignIn({ 'a': 0 }, new Foo, new Bar); // => { 'a': 1, 'b': 2, 'c': 3,
	 * 'd': 4 }
	 */
	 ...
	 * `undefined`, assignment is handled by the method instead. The
	 * `customizer` is invoked with five arguments: (objValue, srcValue, key,
	 * object, source).
	 * 
	 * **Note:** This method mutates `object`.
	 * 
	 ...
	 * @example
	 * 
	 * function customizer(objValue, srcValue) { return _.isUndefined(objValue) ?
	 * srcValue : objValue; }
	 * 
	 ...
	 * Once a property is set, additional values of the same property are
	 * ignored.
	 * 
	 * **Note:** This method mutates `object`.
	 * 
	 ...
	 * 
	 * _.findKey(users, function(o) { return o.age < 40; }); // => 'barney'
	 * (iteration order is not guaranteed) // The `_.matches` iteratee
	 * shorthand. _.findKey(users, { 'age': 1, 'active': true }); // =>
	 * 'pebbles' // The `_.matchesProperty` iteratee shorthand. _.findKey(users,
	 * ['active', false]); // => 'fred' // The `_.property` iteratee shorthand.
	 ...
	 * 40, 'active': false }, 'pebbles': { 'age': 1, 'active': true } };
	 * 
	 * _.findLastKey(users, function(o) { return o.age < 40; }); // => returns
	 * 'pebbles' assuming `_.findKey` returns 'barney' // The `_.matches`
	 * iteratee shorthand. _.findLastKey(users, { 'age': 36, 'active': true }); // =>
	 * 'barney' // The `_.matchesProperty` iteratee shorthand.
	 * _.findLastKey(users, ['active', false]); // => 'fred' // The `_.property`
	 * iteratee shorthand. _.findLastKey(users, 'active'); // => 'pebbles'
	 */
	 ...
	 * @example
	 * 
	 * var object = { 'a': { 'b': 2 } }; var other = _.create({ 'a': _.create({
	 * 'b': 2 }) });
	 * 
	 * _.has(object, 'a'); // => true
	 * 
	 ...
	 * `iteratee`. The corresponding inverted value of each inverted key is an
	 * array of keys responsible for generating the inverted value. The iteratee
	 ...
	 * 
	 * _.invertBy(object, function(value) { return 'group' + value; }); // => {
	 * 'group1': ['a', 'c'], 'group2': ['b'] }
	 */
	 ...
	 * spec](http://ecma-international.org/ecma-262/7.0/#sec-object.keys) for
	 * more details.
	 * 
	 * @static
	 * @since 0.1.0
	 ...
	 * string keyed property of `object` thru `iteratee`. The iteratee is
	 ...
	 * iteratee shorthand. _.mapValues(users, 'age'); // => { 'fred': 40,
	 * 'pebbles': 1 } (iteration order is not guaranteed)
	 */
	 ...
	 * var other = { 'a': [{ 'c': 3 }, { 'e': 5 }] };
	 * 
	 * _.merge(object, other); // => { 'a': [{ 'b': 2, 'c': 3 }, { 'd': 4, 'e':
	 * 5 }] }
	 */
	 ...
	 * @example
	 * 
	 * function customizer(objValue, srcValue) { if (_.isArray(objValue)) {
	 * return objValue.concat(srcValue); } }
	 * 
	 * var object = { 'a': [1], 'b': [2] }; var other = { 'a': [3], 'b': [4] };
	 * 
	 ...
	 * own and inherited enumerable string keyed properties of `object` that
	 ...
	 * 
	 * _.set(object, ['x', '0', 'y', 'z'], 5); console.log(object.x[0].y.z); // =>
	 * 5
	 */
	 ...
	 * Foo.prototype.c = 3;
	 * 
	 * _.toPairs(new Foo); // => [['a', 1], ['b', 2]] (iteration order is not
	 * guaranteed)
	 */
	 ...
	 * Foo.prototype.c = 3;
	 * 
	 * _.toPairsIn(new Foo); // => [['a', 1], ['b', 2], ['c', 3]] (iteration
	 ...
	 * iteratee is invoked with four arguments: (accumulator, value, key,
	 ...
	 * (result[value] || (result[value] = [])).push(key); }, {}); // => { '1':
	 * ['a', 'c'], '2': ['b'] }
	 */
	 ...
	 * var object = {};
	 * 
	 * _.updateWith(object, '[0][1]', _.constant('a'), Object); // => { '0': {
	 * '1': 'a' } }
	 */
	 ...
	 * given number is returned. If `floating` is `true`, or either `lower` or
	 * `upper` are floats, a floating-point number is returned instead of an
	 * integer.
	 * 
	 ...
	 * setting object is given, it takes precedence over `_.templateSettings`
	 * values.
	 * 
	 ...
	 *            [options={}] The options object.
	 * @param {RegExp}
	 *            [options.escape=_.templateSettings.escape] The HTML "escape"
	 *            delimiter.
	 * @param {RegExp}
	 *            [options.evaluate=_.templateSettings.evaluate] The "evaluate"
	 *            delimiter.
	 * @param {Object}
	 ...
	 * @param {string}
	 *            [options.sourceURL='lodash.templateSources[n]'] The sourceURL
	 ...
	 * @example // Use the "interpolate" delimiter to create a compiled
	 *          template. var compiled = _.template('hello <%= user %>!');
	 *          compiled({ 'user': 'fred' }); // => 'hello fred!' // Use the
	 *          HTML "escape" delimiter to escape data property values. var
	 *          compiled = _.template('<b><%- value %></b>'); compiled({
	 *          'value': '<script>' }); // => '<b>&lt;script&gt;</b>' // Use
	 *          the "evaluate" delimiter to execute JavaScript and generate
	 *          HTML. var compiled = _.template('<% _.forEach(users,
	 *          function(user) { %>
	 *          <li><%- user %></li><% }); %>'); compiled({ 'users': ['fred',
	 *          'barney'] }); // => '
	 *          <li>fred</li>
	 *          <li>barney</li>' // Use the internal `print` function in
	 *          "evaluate" delimiters. var compiled = _.template('<%
	 *          print("hello " + user); %>!'); compiled({ 'user': 'barney' }); // =>
	 *          'hello barney!' // Use the ES template literal delimiter as an
	 *          "interpolate" delimiter. // Disable support by replacing the
	 *          "interpolate" delimiter. var compiled = _.template('hello ${
	 *          user }!'); compiled({ 'user': 'pebbles' }); // => 'hello
	 *          pebbles!' // Use backslashes to treat delimiters as plain text.
	 *          var compiled = _.template('<%= "\\<%- value %\\>" %>');
	 *          compiled({ 'value': 'ignored' }); // => '<%- value %>' // Use
	 *          the `imports` option to import `jQuery` as `jq`. var text = '<%
	 *          jq.each(users, function(user) { %>
	 *          <li><%- user %></li><% }); %>'; var compiled =
	 *          _.template(text, { 'imports': { 'jq': jQuery } }); compiled({
	 *          'users': ['fred', 'barney'] }); // => '
	 *          <li>fred</li>
	 *          <li>barney</li>' // Use the `sourceURL` option to specify a
	 *          custom sourceURL for the template. var compiled =
	 *          _.template('hello <%= user %>!', { 'sourceURL':
	 *          '/basic/greeting.jst' }); compiled(data); // => Find the source
	 *          of "greeting.jst" under the Sources tab or Resources panel of
	 *          the web inspector. // Use the `variable` option to ensure a
	 *          with-statement isn't used in the compiled template. var compiled =
	 *          _.template('hi <%= data.user %>!', { 'variable': 'data' });
	 *          compiled.source; // => function(data) { // var __t, __p = ''; //
	 *          __p += 'hi ' + ((__t = ( data.user )) == null ? '' : __t) + '!'; //
	 *          return __p; // } // Use custom template delimiters.
	 *          _.templateSettings.interpolate = /{{([\s\S]+?)}}/g; var compiled =
	 *          _.template('hello {{ user }}!'); compiled({ 'user': 'mustache'
	 *          }); // => 'hello mustache!' // Use the `source` property to
	 *          inline compiled templates for meaningful // line numbers in
	 ...
	 * error object. Any additional arguments are provided to `func` when it's
	 * invoked.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * @example // Avoid throwing errors for invalid selectors. var elements =
	 ...
	 * @example
	 * 
	 * var view = { 'label': 'docs', 'click': function() { console.log('clicked ' +
	 * this.label); } };
	 * 
	 * _.bindAll(view, ['click']); jQuery(element).on('click', view.click); // =>
	 ...
	 * @example
	 * 
	 * var func = _.cond([ [_.matches({ 'a': 1 }), _.constant('matches A')],
	 * [_.conforms({ 'b': _.isNumber }), _.constant('matches B')], [_.stubTrue,
	 * _.constant('no match')] ]);
	 * 
	 ...
	 * its place. The `defaultValue` is returned if `value` is `NaN`, `null`, or
	 * `undefined`.
	 * 
	 * @static
	 * @memberOf _
	 ...
	 * var users = [ { 'user': 'barney', 'age': 36, 'active': true }, { 'user':
	 * 'fred', 'age': 40, 'active': false } ]; // The `_.matches` iteratee
	 * shorthand. _.filter(users, _.iteratee({ 'user': 'barney', 'active': true
	 * })); // => [{ 'user': 'barney', 'age': 36, 'active': true }] // The
	 * `_.matchesProperty` iteratee shorthand. _.filter(users,
	 * _.iteratee(['user', 'fred'])); // => [{ 'user': 'fred', 'age': 40 }] //
	 * The `_.property` iteratee shorthand. _.map(users, _.iteratee('user')); // =>
	 * ['barney', 'fred'] // Create custom iteratee shorthands. _.iteratee =
	 * _.wrap(_.iteratee, function(iteratee, func) { return !_.isRegExp(func) ?
	 * iteratee(func) : function(string) { return func.test(string); }; });
	 * 
	 ...
	 * var objects = [ { 'a': 1, 'b': 2, 'c': 3 }, { 'a': 4, 'b': 5, 'c': 6 } ];
	 * 
	 * _.find(objects, _.matchesProperty('a', 4)); // => { 'a': 4, 'b': 5, 'c':
	 * 6 }
	 */
	 ...
	 * @example
	 * 
	 * var objects = [ { 'a': { 'b': _.constant(2) } }, { 'a': { 'b':
	 * _.constant(1) } } ];
	 * 
	 ...
	 * var objects = [{ 'n': 1 }, { 'n': 2 }];
	 * 
	 * _.maxBy(objects, function(o) { return o.n; }); // => { 'n': 2 } // The
	 * `_.property` iteratee shorthand. _.maxBy(objects, 'n'); // => { 'n': 2 }
	 */
	 ...
	 * 
	 * _.meanBy(objects, function(o) { return o.n; }); // => 5 // The
	 * `_.property` iteratee shorthand. _.meanBy(objects, 'n'); // => 5
	 */
	 ...
	 * var objects = [{ 'n': 1 }, { 'n': 2 }];
	 * 
	 * _.minBy(objects, function(o) { return o.n; }); // => { 'n': 1 } // The
	 * `_.property` iteratee shorthand. _.minBy(objects, 'n'); // => { 'n': 1 }
	 */
	 ...
	 * var objects = [{ 'n': 4 }, { 'n': 2 }, { 'n': 8 }, { 'n': 6 }];
	 * 
	 * _.sumBy(objects, function(o) { return o.n; }); // => 20 // The
	 * `_.property` iteratee shorthand. _.sumBy(objects, 'n'); // => 20
	 */
